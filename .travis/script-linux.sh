...
if [ $CROSS_WINDOWS = true ]; then
    ...
    PATH=$HOME/.local/bin:$PATH ./cross_compile_ffmpeg.sh --build-ffmpeg-shared=y --compiler-flavors=multi             \
        --enable-gpl=y --sandbox-ok=y --ffmpeg-git-checkout-version=${FFMPEG_GIT_TAG}                                  \
        --ffmpeg-git-checkout=${FFMPEG_GIT_REPO} --gcc-cpu-count=2 || exit 1
